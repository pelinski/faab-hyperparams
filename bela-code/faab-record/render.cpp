...
// Osc
float gFrequency[NUM_SENSORS] = {100.0,  200.0,  400.0,  800.0,
...
float gOutputs[NUM_OUTPUTS] = {0.0, 0.0, 0.0, 0.0};
...
bool setup(BelaContext *context, void *userData) {
  ...
  for (unsigned int i = 0; i < NUM_SENSORS; ++i) {
    gFaabWatchers.push_back(
        new Watcher<float>("gFaabSensor_" + std::to_string(i + 1)));
  }
